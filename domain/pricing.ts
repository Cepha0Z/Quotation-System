...
export const itemReferenceRate = (
  ...
  projectTier: Tier,
  forcedTier?: Tier,
) => item.rates[forcedTier ?? effectiveTier(item, projectTier)] ?? 0;
export const itemBaseRate = (
  item: QuoteItem,
  projectTier: Tier,
  forcedTier?: Tier,
) =>
...
export const itemOriginalTotal = (
  item: QuoteItem,
  projectTier: Tier,
  forcedTier?: Tier,
) =>
...
export const itemTotal = (
  ...
  Math.max(
    0,
    itemOriginalTotal(item, projectTier, forcedTier) - safe(item.discount),
  );
export const itemSavings = (
  item: QuoteItem,
  projectTier: Tier,
  forcedTier?: Tier,
) => {
  ...
  return (
    rateSaving +
    Math.min(
      itemOriginalTotal(item, projectTier, forcedTier),
      safe(item.discount),
    )
  );
};
...
export const interiorTotal = (project: Project, forcedTier?: Tier) =>
  project.rooms.reduce(
    (sum, room) => sum + roomTotal(room, project.defaultTier, forcedTier),
    0,
  );
export const feeOriginal = (
  fee: Fee,
  project: Project,
  interior = interiorTotal(project),
) =>
  fee.enabled
    ? fee.method === 'sqft'
      ? safe(project.carpetArea) * safe(fee.value)
      : fee.method === 'percentage'
        ? (interior * safe(fee.value)) / 100
        : safe(fee.value)
    : 0;
export const feeTotal = (
  fee: Fee,
  project: Project,
  interior = interiorTotal(project),
) => Math.max(0, feeOriginal(fee, project, interior) - safe(fee.discount));
export const quoteTotals = (project: Project, forcedTier?: Tier) => {
  const interior = interiorTotal(project, forcedTier);
  const fees = project.fees.map((fee) => ({
    ...fee,
    original: feeOriginal(fee, project, interior),
    total: feeTotal(fee, project, interior),
  }));
  const feeTotalValue = fees.reduce((sum, f) => sum + f.total, 0);
  const subtotal = interior + feeTotalValue;
  const itemSavingsValue = project.rooms.reduce(
    (sum, room) =>
      sum +
      room.items.reduce(
        (roomSum, item) =>
          roomSum + itemSavings(item, project.defaultTier, forcedTier),
        0,
      ),
    0,
  );
  const feeSavings = fees.reduce(
    (sum, fee) => sum + Math.max(0, fee.original - fee.total),
    0,
  );
  const discount = safe(project.projectDiscount);
  return {
    interior,
    fees,
    ...
    subtotal,
    discount,
    savings: itemSavingsValue + feeSavings + discount,
