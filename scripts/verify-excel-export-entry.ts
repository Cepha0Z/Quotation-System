...
const makeItem = (
  ...
  id,
  name,
  description: 'Supply, fabrication and installation as per approved drawings and site measurements.',
  enabled: true,
...
const volumeItems = Array.from({ length: 18 }, (_, index) =>
  makeItem(`volume-${index}`, `Joinery line ${index + 1}`, 'Millwork', {
    ...
    rates: { standard: 875 + index * 25, premium: 1_100 + index * 25, luxury: 1_400 + index * 25 },
...
const project: Project = {
  ...
  rooms: [
    {
      ...
      items: [
        makeItem('long', 'Acoustic feature wall', 'Civil Work', {
          ...
          hsnCode: '995476',
          notes: 'Coordinate junctions with skirting and adjacent stone cladding before execution.',
        }),
        makeItem('zero', 'Client-supplied pendant', 'Bought Out', {
          description: 'Installation allowance for client-supplied decorative pendant.',
    ...
    },
    {
      ...
      items: [
        ...
        makeItem('components', 'Media console', 'Millwork', {
          ...
          subUnits: [
            { id: 'hardware', name: 'Hardware package', rate: 4_800, enabled: true },
            { id: 'optional', name: 'Optional light', rate: 1_200, enabled: false },
          ],
        }),
      ],
    },
  ...
  fees: [
    ...
    { id: 'drawing', name: '3D / Drawing', method: 'flat', value: 35_000, discount: 0, enabled: true },
    { id: 'management', name: 'Project Management', method: 'percentage', value: 6.5, discount: 2_500, enabled: true },
  ],
};

async function main() {
const outputDir = path.resolve('outputs/01a08777-ddc9-7ec1-83de-00418b8e5c44');
...
const expectedSheets = ['Summary', 'Civil Work', 'Millwork', 'Bought Out', 'Flooring'];
...
const findAmount = (sheetName: string, label: string) => {
  ...
  for (let row = range.s.r; row <= range.e.r; row += 1) {
    ...
    if (first === label) return Number(sheet[X.utils.encode_cell({ r: row, c: 6 })]?.v ?? NaN);
  }
  return Number.NaN;
};
...
for (const workType of expectedSheets.slice(1)) {
  ...
  if (actual !== expected) throw new Error(`${workType} total does not reconcile: ${actual} vs ${expected}`);
}

const archive = unzipSync(generated.bytes);
const workbookXml = strFromU8(archive['xl/workbook.xml']);
if (!workbookXml.includes('_xlnm.Print_Area') || !workbookXml.includes('_xlnm.Print_Titles')) {
...
for (let index = 0; index < expectedSheets.length; index += 1) {
  ...
  if (/<pane\b|state="frozen"/.test(xml)) {
    throw new Error(`${expectedSheets[index]} unexpectedly contains frozen panes.`);
  }
  for (const token of ['orientation="landscape"', 'fitToWidth="1"', '<pageMargins']) {
    if (!xml.includes(token)) throw new Error(`${expectedSheets[index]} is missing ${token}.`);
  }
}

const errorPattern = /#(?:REF!|VALUE!|DIV\/0!|NAME\?|N\/A|NUM!|NULL!|SPILL!|CALC!)/;
for (const sheetName of workbook.SheetNames) {
  const rows = X.utils.sheet_to_json<unknown[]>(workbook.Sheets[sheetName], { header: 1, raw: false });
  if (
    rows.some((row) =>
      row.some(
        (value) =>
          (typeof value === 'string' || typeof value === 'number') && errorPattern.test(String(value)),
      ),
    )
  ) {
...
console.log(JSON.stringify({ outputPath, sheets: workbook.SheetNames, totals, bytes: generated.bytes.length }, null, 2));
}
